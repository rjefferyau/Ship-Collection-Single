...
const FranchiseManager: React.FC = () => {
  ...
  const [editingFranchise, setEditingFranchise] = useState<Partial<Franchise> | null>(null);
  ...
  const handleOpenModal = (franchise?: Franchise) => {
    if (franchise) {
      ...
      setEditingFranchise({ name: '', description: '' });
    }
    setShowModal(true);
    setFormError(null);
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setEditingFranchise(prev => prev ? { ...prev, [name]: value } : null);
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    
    if (!editingFranchise || !editingFranchise.name) {
    ...
    try {
      const isEditing = Boolean(editingFranchise._id);
      const url = isEditing 
        ? `/api/franchises/${editingFranchise._id}` 
        : '/api/franchises';
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to save franchise');
      }
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to delete franchise');
      }
  ...
  return (
    <div className="franchise-manager">
      {error && (
      ...
      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        {loading ? (
          ...
          <div className="p-4 text-center">
            <p className="text-gray-600">No franchises found. Add your first one!</p>
          </div>
        ) : (
                  ...
                  <tr key={franchise._id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <div className="flex space-x-2">
                        ...
                        {deleteConfirmation === franchise._id ? (
                          <>
                            <button
                          ...
                          <button
                            className="text-red-600 hover:text-red-900"
                            onClick={() => handleDeleteConfirm(franchise._id)}
                          >
      ...
      {/* Modal for adding/editing franchises */}
      {showModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <h3 className="text-lg font-medium leading-6 text-gray-900">
                {editingFranchise?._id ? 'Edit Franchise' : 'Add Franchise'}
              </h3>
              ...
              <form onSubmit={handleSubmit} className="mt-4">
                <div className="mb-4">
                  <label htmlFor="franchiseName" className="block text-sm font-medium text-gray-700 mb-1">
                    Name
                  </label>
                  <input
                    type="text"
                ...
                <div className="mb-4">
                  <label htmlFor="franchiseDescription" className="block text-sm font-medium text-gray-700 mb-1">
                    Description
                  </label>
                  <textarea
                    id="franchiseDescription"
                    name="description"
                    value={editingFranchise?.description || ''}
                    onChange={handleInputChange}
                    rows={3}
                    className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md"
                  />
